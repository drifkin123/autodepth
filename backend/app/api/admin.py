...
async def bat_url_list() -> list[BatUrlEntry]:
...
async def cars_and_bids_url_list() -> list[CarsAndBidsUrlEntry]:
...
async def get_status() -> ScraperStatus:
    return ScraperStatus(is_running=broadcaster.is_running, effective_secret_hint="")
...
async def get_sales(
    ...
    items = [
        SaleOut(
            id=sale.id,
            car_make=sale.make or (car.make if car else None),
            car_model=sale.model or (car.model if car else None),
            car_trim=sale.trim or (car.trim if car else None),
            ...
            auction_status=sale.auction_status, high_bid=sale.high_bid,
            bid_count=sale.bid_count, title=sale.title, subtitle=sale.subtitle,
            image_count=sale.image_count,
